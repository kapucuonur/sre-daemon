...
class HealingOrchestrator:
    ...
    def _build_prompt(self, tagged_line: str, project_tag: str, err_hash: str = None, context_section: str = "") -> str:
        ...
        return (
            ...
            "- 'replace': Bir dosyada belirli bir kod bloğunu yeni kod bloğuyla değiştirmek için.\n"
            "  Format: {\"type\": \"replace\", \"target\": \"/home/pi/bikefit/pi_main.py\", \"search\": \"eski_kod_satiri\", \"replace\": \"yeni_kod_satiri\"}\n"
    ...
    def _heal(self, tagged_line: str, project_tag: str, err_hash: str):
        ...
        # Traceback ve kod bağlamı analizi (Diagnostic Layer 2)
        file_context = ""
        try:
            ...
            if match:
                ...
                # Docker path translation to Pi physical path
                target_file = raw_file
                if raw_file.startswith("/app/"):
                    proj_clean = project_tag.strip("[]").lower()
                    if "bikefit" in proj_clean:
                        target_file = raw_file.replace("/app/", "/home/pi/bikefit-api/")
                    elif "trihonor" in proj_clean:
                        target_file = raw_file.replace("/app/", "/home/pi/TriHonor-API/")
                    elif "ai-coach" in proj_clean:
                        target_file = raw_file.replace("/app/", "/home/pi/projects/AI-Coach/")
                elif not raw_file.startswith("/home/pi/"):
                    proj_clean = project_tag.strip("[]").lower()
                    if "bikefit" in proj_clean:
                        target_file = "/home/pi/bikefit-api/" + os.path.basename(raw_file)
                    elif "trihonor" in proj_clean:
                        target_file = "/home/pi/TriHonor-API/" + os.path.basename(raw_file)

                logger.info("[DIAGNOSTIC] Traceback algılandı: %s L%d. Kod bağlamı okunuyor...", target_file, line_no)
                
                # Context satırlarını oku (satır -20 ve +20)
                start_line = max(1, line_no - 20)
                end_line = line_no + 20
                raw_code = self.file_editor.read_file(target_file, start_line, end_line)
                if raw_code and not raw_code.startswith("Error"):
                    file_context = (
                        f"HATANIN ALINDIĞI KOD DOSYASI BAĞLAMI ({target_file} satır {start_line}-{end_line}):\n"
                        "```python\n"
                        f"{raw_code}\n"
                        "```\n"
                        "Bu kod bağlamını incele, hata satırını tespit et ve buna uygun bir 'replace' aksiyonu üret.\n\n"
                    )
        except Exception as e_diag:
            logger.warning("[DIAGNOSTIC] Kod bağlamı okunamadı: %s", e_diag)
